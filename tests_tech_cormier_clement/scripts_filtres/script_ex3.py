...
def exercice_trois(enregistrement, position, valeur):
...
#On écrit les données triées dans des fichiers qui seront créés dans le dossier results/filtre/exo3 pour chacune des listes
    sorted_file = open("../results/filtre/exo3/exercice3_a.txt","w")
    sorted_file.write(list_a_form)
    sorted_file.close()

    sorted_file = open("../results/filtre/exo3/exercice3_b.txt","w")
...
if len(sys.argv) != 4:
    ...
    valeur = sys.argv[3]
    exercice_trois(enregistrement, position, valeur)
